...
ExtDef:Specifire ExtDecList SEMI    {
		$$=newAst("ExtDef",3,$1,$2,$3);nodeList[nodeNum]=$$;nodeNum++;
		// 错误类型7:变量出现重复定义
		if(findvar($2)) 
...
StructSpecifire:STRUCT OptTag LC DefList RC {
		// 结构体定义完成，当前在结构体定义外部
		inStruc = 0;
		$$=newAst("StructSpecifire",5,$1,$2,$3,$4,$5);nodeList[nodeNum]=$$;nodeNum++; 
...
Tag:ID {$$=newAst("Tag",1,$1);nodeList[nodeNum]=$$;nodeNum++; }
	;
/*Declarators*/
VarDec:ID {$$=newAst("VarDec",1,$1); $$->tag=1;nodeList[nodeNum]=$$;nodeNum++;}
	|VarDec LB INT RB {$$=newAst("VarDec",4,$1,$2,$3,$4);nodeList[nodeNum]=$$;nodeNum++; $$->content=$1->content;$$->tag=4;}
	;
FunDec:ID LP VarList RP {
		$$=newAst("FunDec",4,$1,$2,$3,$4); $$->content=$1->content;nodeList[nodeNum]=$$;nodeNum++;
	...
	|ID LP RP {
		$$=newAst("FunDec",3,$1,$2,$3);nodeList[nodeNum]=$$;nodeNum++; $$->content=$1->content;
		// 错误类型8:函数出现重复定义（即同样的函数名出现了不止一次定义）
		if(findfunc($1)) 
			printf("Error type 8 at Line %d:Redefined Function '%s'\n",yylineno,$1->content);
		// 设置函数名称以及参数列表
...
ParamDec:Specifire VarDec {
		$$=newAst("ParamDec",2,$1,$2);nodeList[nodeNum]=$$;nodeNum++; 
		// 错误类型7:变量出现重复定义
...
Stmt:Exp SEMI {$$=newAst("Stmt",2,$1,$2);nodeList[nodeNum]=$$;nodeNum++; }
	...
	|RETURN Exp SEMI {
		$$=newAst("Stmt",3,$1,$2,$3);nodeList[nodeNum]=$$;nodeNum++; 
		getrtype($2);
	}
...
Def:Specifire DecList SEMI {
		$$=newAst("Def",3,$1,$2,$3);nodeList[nodeNum]=$$;nodeNum++; 
		// 错误类型7:变量出现重复定义
		if(findvar($2)||findarray($2))  
			printf("Error type 7 at Line %d:Redefined Variable '%s'\n",yylineno,$2->content);
        else if($2->tag==4) 
...
DecList:Dec {$$=newAst("DecList",1,$1);nodeList[nodeNum]=$$;nodeNum++; }
	|Dec COMMA DecList {$$=newAst("DecList",3,$1,$2,$3);nodeList[nodeNum]=$$;nodeNum++; $$->tag=$3->tag;}
	;
Dec:VarDec {$$=newAst("Dec",1,$1);nodeList[nodeNum]=$$;nodeNum++; }
	|VarDec ASSIGNOP Exp {$$=newAst("Dec",3,$1,$2,$3);nodeList[nodeNum]=$$;nodeNum++; $$->content=$1->content;}
	;
/*Expressions*/
Exp:Exp ASSIGNOP Exp {
		$$=newAst("Exp",3,$1,$2,$3);nodeList[nodeNum]=$$;nodeNum++; 
		// 当有一边变量是未定义时，不进行处理
	...
	|ID LP Args RP {
		$$=newAst("Exp",4,$1,$2,$3,$4);nodeList[nodeNum]=$$;nodeNum++; 
	...
	|ID {
		$$=newAst("Exp",1,$1);nodeList[nodeNum]=$$;nodeNum++; 
		// 错误类型1:变量在使用时未经定义
	...
	|INT {$$=newAst("Exp",1,$1);nodeList[nodeNum]=$$;nodeNum++; $$->tag=3;$$->type="int";}
	|FLOAT{$$=newAst("Exp",1,$1);nodeList[nodeNum]=$$;nodeNum++; $$->tag=3;$$->type="float";$$->value=$1->value;}
	;
